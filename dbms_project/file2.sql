...
DELIMITER $$
--FUNCTION WITH JOIN OPERATION

CREATE FUNCTION GetClientDetailsWithSchedules(Client_ID INT)
RETURNS TABLE
BEGIN
    RETURN (
        SELECT 
        ...
        FROM 
            Client c
        JOIN 
            Client_Schedule cs ON c.Client_ID = cs.Client_ID
        JOIN 
            Schedule s ON cs.Schedule_ID = s.Schedule_ID
        JOIN 
            Trainer_Schedule ts ON s.Schedule_ID = ts.Schedule_ID
        JOIN 
            Trainer t ON ts.Trainer_ID = t.Trainer_ID
        WHERE 
